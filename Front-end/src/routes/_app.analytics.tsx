...
function AnalyticsPage() {
  ...
  // ── Chart data ────────────────────────────────────────────
  const vehicleStatusData = useMemo(() => ["Available", "On Trip", "In Shop", "Retired"].map((s) => ({
    name: s,
    value: vehicles.filter((v) => v.status === s).length,
  })), [vehicles]);

  const tripStatusData = useMemo(() => ["Draft", "Dispatched", "Completed", "Cancelled"].map((s) => ({
    name: s,
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={monthlyData} margin={{ top: 5, right: 10, bottom: 0, left: 10 }}>
                ...
                <YAxis tick={{ fontSize: 11, fill: "oklch(0.62 0.01 260)" }} tickFormatter={(v) => `${currencySymbol}${(v / 1000).toFixed(0)}k`} />
