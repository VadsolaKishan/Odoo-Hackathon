        ...
        <CardContent className="p-4 sm:p-6">
          <div className="flex flex-col gap-3 sm:flex-row">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search by name or license…" value={q} onChange={(e) => setQ(e.target.value)} className="pl-9" />
            </div>
                ...
                {filtered.map((d) => {
                  ...
                  return (
                    <TableRow key={d.id} className="hover:bg-muted/50">
                      ...
                      <TableCell>
                        <span className={cn("inline-flex items-center gap-1.5", expired && "text-destructive font-medium")}>
                          {expired && <AlertTriangle className="h-3.5 w-3.5" />}
                          {d.expiry}
                        </span>
                      </TableCell>
                      <TableCell className="text-muted-foreground">{d.phone}</TableCell>
                      <TableCell className={cn("text-right font-semibold tabular-nums", scoreColor(d.safetyScore))}>
                        {d.safetyScore}
                      </TableCell>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-2 gap-4">
            <Field label="Name" className="col-span-2"><Input {...form.register("name")} /></Field>
            <Field label="License Number"><Input {...form.register("license")} /></Field>
            <Field label="Category">
              <Select defaultValue={form.getValues("category")} onValueChange={(v) => form.setValue("category", v as LicenseCategory)}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {["LMV", "HMV", "HTV", "PSV"].map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}
                </SelectContent>
              </Select>
            </Field>
            <Field label="Expiry"><Input type="date" {...form.register("expiry")} /></Field>
            <Field label="Phone"><Input {...form.register("phone")} /></Field>
            <Field label="Safety Score"><Input type="number" {...form.register("safetyScore")} /></Field>
            <Field label="Status">
