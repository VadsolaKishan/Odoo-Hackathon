...
function TripsPage() {
  ...
  const form = useForm<TripForm>({
    ...
    defaultValues: { source: "", destination: "", vehicleId: "", driverId: "", cargoWeight: 100, distance: 100 },
  });
                ...
                <Card key={t.id} className="transition-all hover:shadow-md">
                  ...
                  <CardContent className="space-y-2 text-sm">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Vehicle: <span className="font-mono">{vehicleById[t.vehicleId]?.registration}</span>
                    ...
                    <div className="flex gap-2 pt-2">
                      ...
                      <Button size="sm" variant="outline" onClick={() => { cancelTrip(t.id); toast("Trip cancelled"); }}>
                        Cancel
                      </Button>
                    </div>
                  </CardContent>
                ...
                <TableBody>
                  {drafts.map((t) => (
                    <TableRow key={t.id}>
                      ...
                      <TableCell className="font-mono text-xs">{vehicleById[t.vehicleId]?.registration}</TableCell>
                      <TableCell>{driverById[t.driverId]?.name}</TableCell>
                      <TableCell className="text-right">{t.cargoWeight}kg</TableCell>
                      <TableCell className="text-right">
                        <Button size="sm" onClick={() => handleDispatch(t.id)}>Dispatch</Button>
                        <Button size="sm" variant="ghost" onClick={() => cancelTrip(t.id)}>Cancel</Button>
                      </TableCell>
                    </TableRow>
                  ))}
                  {drafts.length === 0 && (
                    <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-8">No draft trips.</TableCell></TableRow>
                  )}
                </TableBody>
              </Table>
            </CardContent>
      ...
      <Dialog open={openCreate} onOpenChange={setOpenCreate}>
        <DialogContent className="max-w-lg">
          ...
          <form onSubmit={form.handleSubmit(onCreate)} className="grid grid-cols-2 gap-4">
            ...
            <Field label="Destination"><Input {...form.register("destination")} /></Field>
            ...
            <Field label="Driver" className="col-span-2">
              <Select onValueChange={(v) => form.setValue("driverId", v)}>
                ...
                <SelectContent>
                  {eligibleDrivers.filter((d) => d.status === "Available" || d.disabled).map((d) => (
            ...
            {selectedVehicle && (
              ...
              >
                <div className="flex items-center gap-2 font-medium">
                  {overCapacity > 0 ? <AlertTriangle className="h-4 w-4" /> : <CheckCircle2 className="h-4 w-4" />}
            ...
            <DialogFooter className="col-span-2">
              ...
              <Button type="submit" disabled={overCapacity > 0}>Create Trip</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent>
          ...
          <div className="grid gap-4">
            ...
            <Field label="Fuel Used (litres)">
              <Input type="number" value={complete.fuelUsed} onChange={(e) => setComplete((c) => ({ ...c, fuelUsed: e.target.value }))} />
            </Field>
            <Field label="Notes">
              <Textarea value={complete.notes} onChange={(e) => setComplete((c) => ({ ...c, notes: e.target.value }))} />
            </Field>
          </div>
          <DialogFooter>
