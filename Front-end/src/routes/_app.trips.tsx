...
import {
  AlertTriangle, CheckCircle2, Circle, Clock, XCircle, Truck,
...
export default function TripsPage() {
  ...
  const overCapacity      = selectedVehicle && watchedCargo > selectedVehicle.capacity
    ? watchedCargo - selectedVehicle.capacity : 0;
  ...
  // ── Handlers ─────────────────────────────────────────────
  const onCreate = (values: TripForm) => {
    const res = createTrip(values);
              ...
              <div className="flex items-center gap-1">
                {STEPS.map((step, idx) => {
                  ...
                  const counts: Record<TripStatus, number> = {
                    Draft: draftCount, Dispatched: dispatchedCount,
                      ...
                      <div className="flex flex-col items-center gap-1.5 flex-1">
                        <div
                          className={cn(
                            ...
                            isActive ? `${meta.bg} ${meta.color}` : "bg-muted text-muted-foreground/40"
                          )}
                        >
                          {count}
                        </div>
                ...
                <form onSubmit={form.handleSubmit(onCreate)} className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      {form.formState.errors.source && (
                        <p className="mt-0.5 text-[11px] text-destructive">{form.formState.errors.source.message}</p>
                      )}
                    </div>
                    <div>
                      ...
                      {form.formState.errors.destination && (
                        <p className="mt-0.5 text-[11px] text-destructive">{form.formState.errors.destination.message}</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <Select onValueChange={(v) => form.setValue("vehicleId", v)}>
                      ...
                      <SelectContent>
                        {availableVehicles.length === 0 ? (
                          <div className="p-3 text-xs text-muted-foreground text-center">No available vehicles</div>
                    ...
                    {form.formState.errors.vehicleId && (
                      <p className="mt-0.5 text-[11px] text-destructive">{form.formState.errors.vehicleId.message}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <Select onValueChange={(v) => form.setValue("driverId", v)}>
                      ...
                      <SelectContent>
                        {eligibleDrivers.filter((d) => !d.disabled).map((d) => (
                          <SelectItem key={d.id} value={d.id}>
                            {d.name} · {d.category}
                          </SelectItem>
                        ))}
                        {eligibleDrivers.filter((d) => !d.disabled).length === 0 && (
                          <div className="p-3 text-xs text-muted-foreground text-center">No available drivers</div>
                        )}
                      </SelectContent>
                    </Select>
                    {form.formState.errors.driverId && (
                      <p className="mt-0.5 text-[11px] text-destructive">{form.formState.errors.driverId.message}</p>
                    )}
                  </div>
                  ...
                  {selectedVehicle && (
                    ...
                    >
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          <p>Vehicle Capacity: <strong>{selectedVehicle.capacity.toLocaleString("en-IN")} kg</strong></p>
                          <p>Cargo Weight: <strong>{watchedCargo.toLocaleString("en-IN")} kg</strong></p>
                          {overCapacity > 0 ? (
                            <p className="mt-1 font-semibold">
                              ✕ Capacity exceeded by {overCapacity.toLocaleString("en-IN")} kg — dispatch blocked
                            </p>
                          ) : (
...
function TripCard({ trip, vehicle, driver, canDispatch, onDispatch, onCancel, onComplete }: TripCardProps) {
  const etaDate = new Date(trip.eta);
  const now = new Date();
  const diffMin = Math.round((etaDate.getTime() - now.getTime()) / 60000);
  ...
  const etaLabel = (() => {
    ...
    return etaDate.toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" });
  })();
    ...
    <div className={cn(
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 text-xs text-muted-foreground">
          {trip.cargoWeight.toLocaleString("en-IN")} kg · {trip.distance} km
        </p>
      </div>
      ...
      <div className="shrink-0 text-right">
        ...
        <p className={cn(
          ...
          trip.status === "Dispatched" ? "text-blue-400" :
          trip.status === "Completed"  ? "text-success" :
          trip.status === "Cancelled"  ? "text-destructive" :
