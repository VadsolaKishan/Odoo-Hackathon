...
function DriversPage() {
    ...
    return drivers.filter(
      (d) =>
        (!lq ||
          ...
          d.license.toLowerCase().includes(lq) ||
          d.phone.includes(lq)) &&
  ...
  const openEdit = (d: Driver) => {
    setEditing(d);
    // Format date string from backend for date input
    const formattedExpiry = d.expiry ? new Date(d.expiry).toISOString().slice(0, 10) : "";
    form.reset({
      ...d,
  ...
  const onSubmit = async (values: FormValues) => {
    if (editing) {
      const res = await updateDriver(editing.id, values);
      if (!res.ok) {
                      ...
                      <TableRow key={d.id} className="hover:bg-muted/30">
                        ...
                        <TableCell className="text-right">
                          <PermissionGuard
                            ...
                            required="manage"
                            fallback={
                              <span className="text-[11px] text-muted-foreground italic">View</span>
                            }
                          >
                            <Button
                              variant="ghost"
