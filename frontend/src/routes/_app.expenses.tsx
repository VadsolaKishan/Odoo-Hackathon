...
export const Route = createFileRoute("/_app/expenses")({
  head: () => ({ meta: [{ title: "Fuel & Expenses — FleetNova" }] }),
...
function ExpensesPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Fuel & Expenses"
        description="Track fuel consumption and operational spend."
      />

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard
          label="Fuel Cost"
          value={Math.round(totals.fuelCost)}
          suffix="₹"
          icon={Fuel}
          tint="primary"
        />
        <KpiCard
          label="Maintenance"
          value={Math.round(totals.maintCost)}
          suffix="₹"
          icon={Wrench}
          tint="info"
        />
        <KpiCard
          ...
          value={Math.round(totals.otherCost)}
          suffix="₹"
          icon={IndianRupee}
          tint="muted"
        />
        <Card className="p-5 bg-gradient-to-br from-primary/15 to-primary/5 border-primary/30">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            across fuel, maintenance and other spend
          </p>
        </Card>
      </div>
                  ...
                  {fuelLogs.map((f) => (
                    <TableRow key={f.id}>
                      ...
                      <TableCell className="text-right tabular-nums font-medium">
                        ₹ {(f.liters * f.pricePerLiter).toLocaleString()}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
                ...
                <TableBody>
                  {expenses.map((e) => (
                    <TableRow key={e.id}>
                      ...
                      <TableCell className="text-right tabular-nums font-medium">
                        ₹ {(e.toll + e.repair + e.misc).toLocaleString()}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
      ...
      <Dialog open={openFuel} onOpenChange={setOpenFuel}>
        <DialogContent>
          ...
          >
            ...
            <F label="₹ per Litre" className="col-span-2">
              <Input type="number" {...fuelForm.register("pricePerLiter")} />
            </F>
      ...
      <Dialog open={openExp} onOpenChange={setOpenExp}>
        <DialogContent>
          ...
          >
            ...
            <F label="Toll (₹)">
              <Input type="number" {...expForm.register("toll")} />
            </F>
            <F label="Repair (₹)">
              <Input type="number" {...expForm.register("repair")} />
            </F>
            <F label="Misc (₹)" className="col-span-2">
              <Input type="number" {...expForm.register("misc")} />
            </F>
            <DialogFooter className="col-span-2">
